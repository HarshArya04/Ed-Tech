...
export function CourseModal({ course, onClose, onCategoryClick }: CourseModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl bg-neutral-900 border border-neutral-800 shadow-2xl">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-neutral-800 border border-neutral-700 flex items-center justify-center text-neutral-400 hover:text-white hover:border-violet-500/50 transition-all duration-300"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="p-6 md:p-8">
          {/* Stats Row removed per UI update - replaced by action boxes below */}
